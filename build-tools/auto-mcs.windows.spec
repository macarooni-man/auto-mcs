...
a = Analysis(['../src/wrapper.py'],
             pathex=[],
             binaries=[],
             datas = [('./icon.ico', '.'), ('.\\baselib.ams', '.'), ('..\\src\\gui-assets\\icons\\sm\\*', '..\\src\\gui-assets\\icons\\sm')],
...
with open("../src/menu.py", 'r') as f:
    script_contents = f.read()
    [png_list.append(x) for x in findall(r"'(.*?)'", script_contents) if '.png' in x and '{' not in x]
    [png_list.append(x) for x in findall(r'"(.*?)"', script_contents) if '.png' in x and '{' not in x]

exclude_list = [basename(file) for file in glob("..\\src\\gui-assets\\icons\\*") if (basename(file) not in png_list) and ("big" not in file)]
...
a.datas += Tree('..\\src\\gui-assets', prefix='gui-assets', excludes=exclude_list)
